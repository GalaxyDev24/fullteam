...
function GetAssociatedGamesController() {
    var ctrl = new Controller(this);
    this.getAssociatedGames = function(packet) {
        ...
        async.waterfall([
            function(callback) {
                knex('user_location')
                    ...
                    .then(function(results) {
                        if (results.length == 0) {
                            return ctrl.errorCallback(callback, 1, "Cannot find user location");
                        }
            ...
            function(userLat, userLon, callback) {
                knex.select(['games.id', 'games.game_time', 'games.game_title',
                    'games.cost', 'games.picture', 'games.owner_id', 'games.team_id AS team_id',
                    ...
                    .where(function() {
                        this.where('game_players.user_id', packet.userID)
                            .orWhere('games.owner_id', packet.userID)
                    })
...
module.exports = new GetAssociatedGamesController();
